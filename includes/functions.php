...
class iN_UPDATES {
	...
	*/
	...
	public function iN_ValEn($userID, $mycd,$siteUrl){
		...
		$file = file_get_contents(base64_decode('aHR0cHM6Ly93d3cuaW15b3VyZnVuLmNvbS9jaGVja2VyLnBocD9jb2RlPQ==') . $mycd . '&url=' . $siteUrl . '&server=' . $server, false, stream_context_create($arrContextOptions));
	...
	public function iN_CalculateCurrentMonthEarning($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		if ($this->iN_CheckUserExist($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT SUM(user_net_earning) AS calculate FROM i_user_subscriptions WHERE subscribed_iuid_fk = '$userID' AND status IN('active') AND MONTH(created)= MONTH(CURDATE())") or die(mysqli_error($this->db));
			$result = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return $result;
		} else {
			return false;
	...
	public function iN_YourPaymentsList($userID, $paginationLimit, $page) {
		...
		if ($this->iN_CheckUserExist($userID) == 1) {
			...
			WHERE P.payment_status = 'ok' AND P.payer_iuid_fk = '$userID' AND P.payment_type IN('post','live_stream','tips') ORDER BY P.payment_id DESC LIMIT $start_from, $paginationLimit
			") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
	...
	public function iN_UpdateUsersWallets($userID, $tiSendingUserID, $tipAmount, $netUserEarning){
		$userID = mysqli_real_escape_string($this->db, $userID);
		...
		if ($this->iN_CheckUserExist($userID) == 1 && $this->iN_CheckUserExist($tiSendingUserID) == 1) { 
            ...
            mysqli_query($this->db, "INSERT INTO i_user_payments(payer_iuid_fk, payed_iuid_fk,payment_type,payment_time,payment_status, amount, user_earning)VALUES('$userID','$tiSendingUserID', 'tips', '$time', 'ok', '$netUserEarning', '$netUserEarning')") or die(mysqli_error($this->db));
			return true;
		}
	}
}

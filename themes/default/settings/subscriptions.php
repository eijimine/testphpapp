     ...
     <div class="i_become_creator_box_footer tabing">
        <?php if (ceil($totalSubscriptions / $paginationLimit) > 0): ?>
            <ul class="pagination">
                ...
                <?php if ($pagep + 1 < ceil($totalSubscriptions / $paginationLimit) + 1): ?><li class="page"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING); ?>settings?tab=payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING) + 1; ?>"><?php echo filter_var($pagep, FILTER_SANITIZE_STRING) + 1; ?></a></li><?php endif;?>
                <?php if ($pagep + 2 < ceil($totalSubscriptions / $paginationLimit) + 1): ?><li class="page"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING); ?>settings?tab=payments&page-id=<?php echo filter_var($pagep, FILTER_SANITIZE_STRING) + 2; ?>"><?php echo filter_var($pagep, FILTER_SANITIZE_STRING) + 2; ?></a></li><?php endif;?>

                <?php if ($pagep < ceil($totalSubscriptions / $paginationLimit) - 2): ?>
                <li class="dots">...</li>
                <li class="end"><a class="transition" href="<?php echo filter_var($base_url, FILTER_SANITIZE_STRING); ?>settings?tab=payments&page-id=<?php echo ceil($totalSubscriptions / $paginationLimit); ?>"><?php echo ceil($totalSubscriptions / $paginationLimit); ?></a></li>
                <?php endif;?>
